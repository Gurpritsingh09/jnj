...
set -e
azure_dbname=$(grep azure_dbname ./config.properties | cut -d"=" -f2)
app_dbname=$(grep app_dbname ./config.properties | cut -d"=" -f2)
app_dbuser=$(grep app_dbuser ./config.properties | cut -d"=" -f2)
appteam_name=$(grep appteam_name ./config.properties | cut -d"=" -f2)
keyvault_name=$(grep keyvault_name ./config.properties | cut -d"=" -f2)
dbadmin_user=$(grep dbadmin_user ./config.properties | cut -d"=" -f2)
group_name=$(grep group_name ./config.properties | cut -d"=" -f2)
