...
set -e
azure_dbname=$(grep azure_dbname ./symfony-qa/config.properties | cut -d"=" -f2)
app_dbname=$(grep app_dbname ./symfony-qa/config.properties | cut -d"=" -f2)
app_dbuser=$(grep app_dbuser ./symfony-qa/config.properties | cut -d"=" -f2)
appteam_name=$(grep appteam_name ./symfony-qa/config.properties | cut -d"=" -f2)
keyvault_name=$(grep keyvault_name ./symfony-qa/config.properties | cut -d"=" -f2)
dbadmin_user=$(grep dbadmin_user ./symfony-qa/config.properties | cut -d"=" -f2)
group_name=$(grep group_name ./symfony-qa/config.properties | cut -d"=" -f2)
