...
set -e
azure_dbname=$(grep azure_dbname ./flyway-qa/config.properties | cut -d"=" -f2)
app_dbname=$(grep app_dbname ./flyway-qa/config.properties | cut -d"=" -f2)
app_dbuser=$(grep app_dbuser ./flyway-qa/config.properties | cut -d"=" -f2)
appteam_name=$(grep appteam_name ./flyway-qa/config.properties | cut -d"=" -f2)
keyvault_name=$(grep keyvault_name ./flyway-qa/config.properties | cut -d"=" -f2)
dbadmin_user=$(grep dbadmin_user ./flyway-qa/config.properties | cut -d"=" -f2)
group_name=$(grep group_name ./flyway-qa/config.properties | cut -d"=" -f2)
