...
variable "aflm_storage_mb" {
  type        = number
  description = "Max storage allowed for a server. Possible values are between 5120 MB(5GB) and 1048576 MB(1TB) for the Basic SKU and between 5120 MB(5GB) and 4194304 MB(4TB) for General Purpose/Memory Optimized SKUs."
}

variable "aflm_backup_retention_days" {
  type        = number
  description = "Backup retention days for the server, supported values are between 7 and 35 days."
}

variable "aflm_geo_redundant_backup_enabled" {
  type        = bool
...
variable "aflm_geo_redundant_backup" {
  type        = string
  description = "Enable Geo-redundant or not for server backup. Valid values for this property are Enabled or Disabled, not supported for the basic tier."
}

variable "aflm_auto_grow_enabled" {
  type        = bool
...
variable "aflm_private_endpoint_connection_enabled" {
  type        = bool
...
variable "aflm_mysql_admin_azure_ad_user" {
  type        = string
  description = "The administrator azure AD user of Airflow Metadata MySQL Server"
}
